...
//		sprintf(&Line[row][0], "DevID:%02d Batt low!", this_device);
	...
	{
		sprintf(&Line[row][0], "%d %5s pDop:%2d.%02d", main_flags.fix_valid, fixType[pp_devices_menu[this_device]->fix_type_opt],
...
void draw_this_device(void)
{
	...
	if(pp_devices_menu[this_device]->valid_date_flag)	//bit 5: information about UTC Date and Time of Day validity conﬁrmation is available
	{
		...
		sprintf(&Line[2][0], "%2d %3d.%03dMHz SF%02d", p_settings_menu->freq_channel,
		...
		sprintf(&Line[4][0], "CPU: %02d%%C %d.%02dVolt",pp_devices_menu[this_device]->core_temperature,
													 (pp_devices_menu[this_device]->core_voltage+270)/100,
				  	  	  	  	  	  	  	  	  	 (pp_devices_menu[this_device]->core_voltage+270)%100);
		sprintf(&Line[5][0], "Battery:  %d.%02dVolt", (pp_devices_menu[this_device]->batt_voltage+270)/100,
 	  	  	  	  	  	  	  	   	   	   	   	    (pp_devices_menu[this_device]->batt_voltage+270)%100);
		...
		sprintf(&Line[11][0], "HeightMSL:  %4dm", ((PVTbuffer[39+6]<<24)+(PVTbuffer[38+6]<<16)+(PVTbuffer[37+6]<<8)+PVTbuffer[36+6])/1000);
		sprintf(&Line[12][0], "GNSSspeed:%3dkm/h", pp_devices_menu[this_device]->gps_speed);

		if(pp_devices_menu[this_device]->gps_speed > GPS_SPEED_THRS) {
				sprintf(&Line[13][0], "HeadingGNSS:%04d%% ", pp_devices_menu[this_device]->gps_heading);
		}else 	sprintf(&Line[13][0], " No Magn Compass");
...
//	sprintf(&Line[7][0], "Azimuth_u : %03d%%", azimuth_deg_unsigned[current_device]);
		...
		sprintf(&Line[11][0], "RX%d to TX%d: %4dmS", pp_devices_menu[current_device]->device_num, this_device, main_flags.endRX_2_TX);
...
//	(pp_devices_menu[this_device]->beeper_flag)?    sprintf(&Line[6][0], " Beeper  on"): sprintf(&Line[6][0], " Beeper off");
	if(pp_devices_menu[this_device]->beeper_flag == 0) sprintf(&Line[6][0], "           ");
    ...
    {//SF12 only: invert flags to transmit in the slot, beacon can get: slot1 for beacon2, slot2 for beacon1
		if(pp_devices_menu[this_device]->beeper_flag == 2) sprintf(&Line[6][0], "           ");
		else if(pp_devices_menu[this_device]->beeper_flag == 1) sprintf(&Line[6][0], "           ");
	}
	else sprintf(&Line[6][0], "           ");

	(pp_devices_menu[this_device]->flwtrek_flag)?   sprintf(&Line[7][0], "           "): sprintf(&Line[7][0], "           ");
