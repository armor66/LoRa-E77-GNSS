...
void SUBGRF_SetRegulatorMode( void )
{
    ...
    if ( ( 1UL == BSP_RADIO_IsDCDC() ) && ( 1UL == DCDC_ENABLE ) )
    {
        mode = USE_DCDC ;
    }
    else
...
void SUBGRF_SetTxParams( uint8_t paSelect, int8_t power, RadioRampTimes_t rampTime )
{
    ...
    if (paSelect == RFO_LP)
    {
        max_power = BSP_RADIO_GetRFOMaxPowerConfig(RADIO_RFO_LP_MAXPOWER);
        if (power >  max_power)
        {
          power = max_power;
        }
    ...
    else /* rfo_hp*/
    {
        ...
        /* WORKAROUND END*/
        max_power = BSP_RADIO_GetRFOMaxPowerConfig(RADIO_RFO_HP_MAXPOWER);
        if (power > max_power)
        {
            power = max_power;
        }
...
static void MEM_set_8( void *dst, uint8_t value, uint16_t size )
{
...
void SUBGRF_GetPacketStatus( PacketStatus_t *pktStatus )
{
    ...
    switch( pktStatus->packetType )
    {
        ...
        case PACKET_TYPE_NONE:
        	...
        	MEM_set_8( pktStatus, 0, sizeof( PacketStatus_t ) );
...
{
	BSP_RADIO_Switch_TypeDef state = RADIO_SWITCH_RX;
