...
void manage_trekpoints(uint8_t range_ind)	//	if(pp_trekpoints[nearest_trekpoint_idx]->distance < 2000)
{
	...
	if(trek_direction_old)		//erase line and distance
	{
		...
		draw_str_by_rows(31, 7*18, &string_buffer[7][3], &Font_11x18, BLACK,BLACK);
	}
	...
	if(pp_trekpoints[nearest_trekpoint_idx]->distance < 1900)
	{
		...
		sprintf(&string_buffer[6][4], "%3dm ", (uint16_t)pp_trekpoints[nearest_trekpoint_idx]->distance);
		draw_str_by_rows(48, 6*19, &string_buffer[6][4], &Font_11x18, WHITE,BLACK);
	...
	}else	//or if(pp_trekpoints[nearest_trekpoint_idx]->distance > 1900)
	{
		...
		sprintf(&string_buffer[6][2], "%4dm ", (uint16_t)pp_trekpoints[nearest_trekpoint_idx]->distance);
		draw_str_by_rows(26, 6*19, &string_buffer[6][2], &Font_11x18, WHITE,BLACK);
	}

	sprintf(&string_buffer[7][4], "%d", nearest_trekpoint_idx);		//"%dm", (uint16_t)pp_trekpoints[nearest_trekpoint_idx]->distance);
	draw_str_by_rows(54, 7*19, &string_buffer[7][4], &Font_11x18, WHITE,BLACK);
}
