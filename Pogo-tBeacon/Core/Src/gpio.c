...
void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, LED_B_Pin|BUZZ_Pin|LED_G_Pin|SW_OUT_Pin
                          |CS_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, LED_R_Pin|RF_CTRL1_Pin|RF_CTRL2_Pin|LED_W_Pin
  ...
  GPIO_InitStruct.Pin = BUZZ_Pin|SW_OUT_Pin|CS_Pin;
  ...
  GPIO_InitStruct.Pin = LED_W_Pin|DC_Pin|RST_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
...
//    //PA4 - GPS PPS interrupt on rising edge
...
void led_w_off(void){
	...
	GPIOB->BSRR = GPIO_BSRR_BR5;	//BUZZ
}

void lcd_on(void){
	SW_OUT_GPIO_Port->BSRR = SW_OUT_Pin;
}
void lcd_off(void){
	SW_OUT_GPIO_Port->BSRR = (SW_OUT_Pin << 16);
}
