...
//set 57600
uint8_t set_baudrate[] = {0xB5, 0x62, 0x06, 0x8A, 0x0C, 0x00, 0x00, 0x07, 0x00, 0x00, 0x01, 0x00, 0x52, 0x40, 0x00, 0xE1, 0x00, 0x00, 0x17, 0x42};
//ACK-ACK	B5 62 05 01 02 00 06 8A 98 C1
uint8_t revert_baudrate[] = {0};	//todo add set baud rate 38400
uint8_t save_to_bbr_flash[] = {0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x1D, 0xAB};
uint8_t revert_to_default[] = {0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x03, 0x1B, 0x9A};
//uint8_t factory_reset[] =   {0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x07, 0x1F, 0x9E};
//ACK-ACK	B5 62 05 01 02 00 06 09 17 40
//set to FLASH, BBR, RAM: NAV-PVT (uart1), pedestrian (3), MEAS (1000), LEN_TP1 (1000), UBX (true), NMEA (false)
uint8_t set_crucial_opts[] = {0xB5, 0x62, 0x06, 0x8A, 0x26, 0x00, 0x00, 0x07, 0x00, 0x00, 0x07, 0x00, 0x91, 0x20, 0x01, 0x21, 0x00, 0x11, 0x20, 0x03, 0x01, 0x00,
			0x21, 0x30, 0xE8, 0x03, 0x04, 0x00, 0x05, 0x40, 0xE8, 0x03, 0x00, 0x00, 0x01, 0x00, 0x74, 0x10, 0x01, 0x02, 0x00, 0x74, 0x10, 0x00, 0x48, 0x34};
//ACK-ACK B5 62 05 01 02 00 06 8A 98 C1
//request from RAM:
//CFG-MSGOUT-UBX_NAVPVT_UART1
uint8_t req_nav_pvt_ram[] = {0xB5, 0x62, 0x06, 0x8B, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0x00, 0x91, 0x20, 0x51, 0xEF};
//answer					 0xB5, 0x62, 0x06, 0x8B, 0x09, 0x00, 0x01, 0x00, 0x00, 0x00, 0x07, 0x00, 0x91, 0x20, 0x01, 0x54, 0x55
//CFG-UART1OUTPROT-UBX
uint8_t req_out_ubx_ram[] = {0xB5, 0x62, 0x06, 0x8B, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x74, 0x10, 0x1E, 0x8D};
//answer					 0xB5, 0x62, 0x06, 0x8B, 0x09, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x74, 0x10, 0x01, 0x21, 0xC0
//CFG-UART1OUTPROT-NMEA
uint8_t req_out_nmea_ram[] = {0xB5, 0x62, 0x06, 0x8B, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x74, 0x10, 0x1F, 0x91};
//answer					 0xB5, 0x62, 0x06, 0x8B, 0x09, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x74, 0x10, 0x00, 0x21, 0xC4
//set BeiDou
//	B5 62 06 3E 34 00 00 00 31 06 00 08 10 00 01 00
//	01 01 01 03 03 00 01 00 01 01 02 08 0C 00 01 00
//	01 01 03 06 0E 00 01 00 01 01 05 03 04 00 01 00
//	05 01 06 08 0C 00 00 00 00 01 35 78
//set GLONASS
//	B5 62 06 3E 34 00 00 31 31 06 00 08 10 00 01 00
//	01 01 01 03 03 00 01 00 01 01 02 08 0C 00 01 00
...
void serialPrint(uint8_t *pBuffer, uint8_t size) {
...
//			sprintf(&Line[row+1][0], "    Current");
		...
		if(1)
		{
			sprintf(&Line[row][0], "   Press OK to   ");
...
//			sprintf(&Line[row+=1][0], "     RE-CHECK    ");
			sprintf(&Line[row+=1][0], "  SET BAUD RATE   ");
			ST7735_WriteString(0, (row)*14+5, &Line[row][0], Font_7x10, ORANGE,BLACK);

			sprintf(&Line[row+=1][0], "   Press PWR to  ");
			ST7735_WriteString(0, (row)*14+7, &Line[row][0], Font_7x10, YELLOW,BLACK);
			sprintf(&Line[row+=1][0], " REVERT DEFAULTS ");
			ST7735_WriteString(0, (row)*14+5, &Line[row][0], Font_7x10, ORANGE,BLACK);
		}
...
#else
		if(!(GPIOB->IDR & BTN_1_Pin))	//PWR for defaults
#endif
		{
