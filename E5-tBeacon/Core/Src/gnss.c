...
//    while (*pBuffer) {
        USART1->TDR = (*pBuffer++ & (uint16_t)0x01FF);
...
void restart_uart(int8_t baud_rate_index)
{
	main_flags.uartIdx = 0;
  	USART1->RDR;
   	...
   	USART1->BRR = baudBRR[baud_rate_index];
   	USART1->CR1 = USART_CR1_UE;
   	USART1->CR1 |= USART_CR1_RE | USART_CR1_TE | USART_CR1_RXNEIE_RXFNEIE;
}
...
restart_configuration:
	...
	for(baudRateInd = 0; baudRateInd < (5 + 1); baudRateInd++)
    {
		restart_uart(baudRateInd);
       	if(HAL_UART_GetState(&huart1) != HAL_UART_STATE_BUSY_RX)	//if gps module does not transmit
       	{
...
//			HAL_Delay(100);
		...
		if(!(GPIOC->IDR & BTN_1_Pin))	//PWR for restart
		{
			//re-init gnss module:
			send_ubx(UBX_CLASS_CFG, UBX_CFG_RST, &cfg_rst_cold_restart[0], sizeof(cfg_rst_cold_restart));
			HAL_Delay(500);
